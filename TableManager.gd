...
@onready var hint_box = $HintBox # 确保编辑器里有这个节点！
...
func _ready():
	...
	hint_box.global_position = (screen_size / 2.0) - (hint_box.size * hint_box.get_global_transform().get_scale() / 2.0)
...
func add_card_to_table(card_node: Node2D):
	if hint_box.visible: hint_box.visible = false # 放入第一张牌后隐藏虚框
	...
	var row = int(total_count / float(cards_per_row))
	...
	if row > current_row_index:
		...
		shift_tween.tween_property(card_container, "position:y", card_container.position.y - 220, 0.4)

	# 核心修复：计算屏幕中心位置，实现“之”字型行居中
	var screen_center = get_viewport_rect().size / 2.0
	var spacing_x = 180.0 # 根据缩放后的卡牌宽度调整
	var row_width = (cards_per_row - 1) * spacing_x
	var start_x = screen_center.x - (row_width / 2.0)
	
	# 使用 global_position 确保不受父节点坐标干扰
